Last Updated: 2021-12-29
Type: #documentation
Tags: [[Linux]], [[Ubuntu]]

# Tools

# [Ventoy](https://www.ventoy.net/en/doc_news.html)
- A USB bootable disk can store multiple OS ISO for booting/installing
- No more of preparing USB boot drivers for different systems
- One single USB drive to boot all the OS

# [I3-wm](https://i3wm.org/) 
- window tiling manager and it have been my chosen user interface for the past 4 years.
- Fully configurable with a single `~/.config/i3/config` file, where every action can be remapped to different key/combo
- With such high level of control and manipulation, my productivity is super accelerated

# [Hunspells](https://hunspell.github.io/)
- Spell checking system for all software

# [autorandr](https://github.com/phillipberndt/autorandr)
- Setup monitor profiles based on current equipment setup and autodetect changes and switch to monitor profile

# [Pyvis](https://pyvis.readthedocs.io/en/latest/index.html)
- Interactive network visualizations tool